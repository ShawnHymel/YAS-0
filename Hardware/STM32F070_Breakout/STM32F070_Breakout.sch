EESchema Schematic File Version 4
LIBS:STM32F070_Breakout-cache
EELAYER 29 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title "YAS-0 Board"
Date "2019-05-30"
Rev "v02"
Comp ""
Comment1 ""
Comment2 "creativecommons.org/licenses/by/4.0"
Comment3 "License: CC BY 4.0"
Comment4 "Author: Shawn Hymel"
$EndDescr
$Comp
L Device:Polyfuse F1
U 1 1 5CF03193
P 1900 1250
F 0 "F1" V 1675 1250 50  0000 C CNN
F 1 "500mA" V 1766 1250 50  0000 C CNN
F 2 "footprints:0ZCK0050FF2E" H 1950 1050 50  0001 L CNN
F 3 "~" H 1900 1250 50  0001 C CNN
	1    1900 1250
	0    1    1    0   
$EndComp
$Comp
L Regulator_Linear:AP2112K-3.3 U1
U 1 1 5CF03C56
P 2900 1350
F 0 "U1" H 2900 1692 50  0000 C CNN
F 1 "AP2112K-3.3" H 2900 1601 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-23-5" H 2900 1675 50  0001 C CNN
F 3 "https://www.diodes.com/assets/Datasheets/AP2112.pdf" H 2900 1450 50  0001 C CNN
	1    2900 1350
	1    0    0    -1  
$EndComp
$Comp
L Device:C C4
U 1 1 5CF04C4A
P 2350 1500
F 0 "C4" H 2465 1546 50  0000 L CNN
F 1 "1uF" H 2465 1455 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 2388 1350 50  0001 C CNN
F 3 "~" H 2350 1500 50  0001 C CNN
	1    2350 1500
	1    0    0    -1  
$EndComp
$Comp
L Device:C C5
U 1 1 5CF05356
P 3300 1500
F 0 "C5" H 3415 1546 50  0000 L CNN
F 1 "1uF" H 3415 1455 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 3338 1350 50  0001 C CNN
F 3 "~" H 3300 1500 50  0001 C CNN
	1    3300 1500
	1    0    0    -1  
$EndComp
$Comp
L Device:R R2
U 1 1 5CF05ACA
P 7150 1250
F 0 "R2" H 7220 1296 50  0000 L CNN
F 1 "1k" H 7220 1205 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 7080 1250 50  0001 C CNN
F 3 "~" H 7150 1250 50  0001 C CNN
	1    7150 1250
	1    0    0    -1  
$EndComp
$Comp
L Device:R R3
U 1 1 5CF05FEB
P 7700 1250
F 0 "R3" H 7770 1296 50  0000 L CNN
F 1 "1k" H 7770 1205 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 7630 1250 50  0001 C CNN
F 3 "~" H 7700 1250 50  0001 C CNN
	1    7700 1250
	1    0    0    -1  
$EndComp
$Comp
L Device:LED D1
U 1 1 5CF066ED
P 7150 1650
F 0 "D1" V 7189 1533 50  0000 R CNN
F 1 "GREEN" V 7098 1533 50  0000 R CNN
F 2 "digikey-footprints:LED_0603" H 7150 1650 50  0001 C CNN
F 3 "~" H 7150 1650 50  0001 C CNN
	1    7150 1650
	0    -1   -1   0   
$EndComp
$Comp
L Device:LED D2
U 1 1 5CF071CB
P 7700 1700
F 0 "D2" V 7739 1583 50  0000 R CNN
F 1 "RED" V 7648 1583 50  0000 R CNN
F 2 "digikey-footprints:LED_0603" H 7700 1700 50  0001 C CNN
F 3 "~" H 7700 1700 50  0001 C CNN
	1    7700 1700
	0    -1   -1   0   
$EndComp
$Comp
L Jumper:SolderJumper_2_Bridged JP3
U 1 1 5CF0794D
P 7700 2100
F 0 "JP3" V 7654 2168 50  0000 L CNN
F 1 "SolderJumper_2_Bridged" V 7745 2168 50  0000 L CNN
F 2 "Jumper:SolderJumper-2_P1.3mm_Bridged_RoundedPad1.0x1.5mm" H 7700 2100 50  0001 C CNN
F 3 "~" H 7700 2100 50  0001 C CNN
	1    7700 2100
	0    1    1    0   
$EndComp
Text Notes 8100 1800 0    50   ~ 0
Power LED\nCut jumper to disable
Text GLabel 1350 1450 2    50   Input ~ 0
USB_D+
Text GLabel 1350 1350 2    50   Input ~ 0
USB_D-
Wire Wire Line
	1250 1250 1750 1250
Wire Wire Line
	2050 1250 2200 1250
Wire Wire Line
	2600 1350 2500 1350
Wire Wire Line
	2500 1350 2500 1250
Connection ~ 2500 1250
Wire Wire Line
	2500 1250 2600 1250
Wire Wire Line
	2350 1250 2350 1350
Connection ~ 2350 1250
Wire Wire Line
	2350 1250 2500 1250
Wire Wire Line
	3200 1250 3300 1250
Wire Wire Line
	3300 1250 3300 1350
Wire Wire Line
	3300 1250 3300 1150
Connection ~ 3300 1250
$Comp
L power:GND #PWR06
U 1 1 5CF1A829
P 2350 1750
F 0 "#PWR06" H 2350 1500 50  0001 C CNN
F 1 "GND" H 2355 1577 50  0000 C CNN
F 2 "" H 2350 1750 50  0001 C CNN
F 3 "" H 2350 1750 50  0001 C CNN
	1    2350 1750
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR08
U 1 1 5CF1B146
P 2900 1750
F 0 "#PWR08" H 2900 1500 50  0001 C CNN
F 1 "GND" H 2905 1577 50  0000 C CNN
F 2 "" H 2900 1750 50  0001 C CNN
F 3 "" H 2900 1750 50  0001 C CNN
	1    2900 1750
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR010
U 1 1 5CF1B368
P 3300 1750
F 0 "#PWR010" H 3300 1500 50  0001 C CNN
F 1 "GND" H 3305 1577 50  0000 C CNN
F 2 "" H 3300 1750 50  0001 C CNN
F 3 "" H 3300 1750 50  0001 C CNN
	1    3300 1750
	1    0    0    -1  
$EndComp
Wire Wire Line
	2350 1650 2350 1750
Wire Wire Line
	2900 1650 2900 1750
Wire Wire Line
	3300 1650 3300 1750
$Comp
L power:+3.3V #PWR09
U 1 1 5CF1C2BF
P 3300 1150
F 0 "#PWR09" H 3300 1000 50  0001 C CNN
F 1 "+3.3V" H 3315 1323 50  0000 C CNN
F 2 "" H 3300 1150 50  0001 C CNN
F 3 "" H 3300 1150 50  0001 C CNN
	1    3300 1150
	1    0    0    -1  
$EndComp
Wire Wire Line
	2200 1250 2200 1000
Wire Wire Line
	2200 1000 2300 1000
Connection ~ 2200 1250
Wire Wire Line
	2200 1250 2350 1250
Text GLabel 2300 1000 2    50   Input ~ 0
VIN
$Comp
L power:+3.3V #PWR019
U 1 1 5CF1E6D2
P 7700 1000
F 0 "#PWR019" H 7700 850 50  0001 C CNN
F 1 "+3.3V" H 7715 1173 50  0000 C CNN
F 2 "" H 7700 1000 50  0001 C CNN
F 3 "" H 7700 1000 50  0001 C CNN
	1    7700 1000
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR018
U 1 1 5CF212B2
P 7150 1900
F 0 "#PWR018" H 7150 1650 50  0001 C CNN
F 1 "GND" H 7155 1727 50  0000 C CNN
F 2 "" H 7150 1900 50  0001 C CNN
F 3 "" H 7150 1900 50  0001 C CNN
	1    7150 1900
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR020
U 1 1 5CF21BDE
P 7700 2350
F 0 "#PWR020" H 7700 2100 50  0001 C CNN
F 1 "GND" H 7705 2177 50  0000 C CNN
F 2 "" H 7700 2350 50  0001 C CNN
F 3 "" H 7700 2350 50  0001 C CNN
	1    7700 2350
	1    0    0    -1  
$EndComp
Wire Wire Line
	7150 1000 7150 1100
Wire Wire Line
	7150 1400 7150 1500
Wire Wire Line
	7150 1800 7150 1900
Wire Wire Line
	7700 1000 7700 1100
Wire Wire Line
	7700 1400 7700 1550
Wire Wire Line
	7700 1850 7700 1950
Wire Wire Line
	7700 2250 7700 2350
Wire Wire Line
	7150 1000 7050 1000
Text GLabel 7050 1000 0    50   Input ~ 0
D20
$Comp
L Connector_Generic:Conn_01x05 J4
U 1 1 5CF16C78
P 10550 1450
F 0 "J4" H 10630 1492 50  0000 L CNN
F 1 "Conn_01x05" H 10630 1401 50  0000 L CNN
F 2 "STM32F070_Breakout:PinHeader_1x05_P2.54mm_Vertical_NoSilk" H 10550 1450 50  0001 C CNN
F 3 "~" H 10550 1450 50  0001 C CNN
F 4 "DNP" H 10700 1600 50  0000 C CNN "Config"
	1    10550 1450
	1    0    0    -1  
$EndComp
Wire Wire Line
	10350 1250 9850 1250
$Comp
L power:+3.3V #PWR026
U 1 1 5CF19D49
P 9850 1150
F 0 "#PWR026" H 9850 1000 50  0001 C CNN
F 1 "+3.3V" H 9865 1323 50  0000 C CNN
F 2 "" H 9850 1150 50  0001 C CNN
F 3 "" H 9850 1150 50  0001 C CNN
	1    9850 1150
	1    0    0    -1  
$EndComp
Wire Wire Line
	9850 1250 9850 1150
Wire Wire Line
	10350 1650 10250 1650
Wire Wire Line
	10350 1550 10250 1550
Wire Wire Line
	10350 1350 10250 1350
Text GLabel 10250 1350 0    50   Input ~ 0
SWCLK
Text GLabel 10250 1650 0    50   Input ~ 0
RESET
Text GLabel 10250 1550 0    50   Input ~ 0
SWDIO
$Comp
L power:GND #PWR027
U 1 1 5CF1DF4A
P 9850 1550
F 0 "#PWR027" H 9850 1300 50  0001 C CNN
F 1 "GND" H 9855 1377 50  0000 C CNN
F 2 "" H 9850 1550 50  0001 C CNN
F 3 "" H 9850 1550 50  0001 C CNN
	1    9850 1550
	1    0    0    -1  
$EndComp
Wire Wire Line
	9850 1450 9850 1550
Wire Wire Line
	9850 1450 10350 1450
Text Notes 9400 600  0    50   ~ 0
SWD Header
Text Notes 6650 600  0    50   ~ 0
LEDs
Text Notes 550  600  0    50   ~ 0
USB and Power
Wire Wire Line
	9700 3900 9600 3900
Wire Wire Line
	9700 4000 9600 4000
Wire Wire Line
	9700 4100 9600 4100
Wire Wire Line
	9700 4200 9600 4200
Wire Wire Line
	9700 4300 9600 4300
Wire Wire Line
	9700 4400 9600 4400
Wire Wire Line
	9700 4500 9600 4500
Wire Wire Line
	9700 4600 9600 4600
Wire Wire Line
	9700 4700 9600 4700
Wire Wire Line
	9700 4800 9600 4800
Wire Wire Line
	9700 4900 9600 4900
Wire Wire Line
	9700 5000 9600 5000
Wire Wire Line
	9700 5100 9600 5100
Wire Wire Line
	9700 5200 9600 5200
Wire Wire Line
	10450 5200 10550 5200
Wire Wire Line
	10450 5100 10550 5100
Wire Wire Line
	10450 5000 10550 5000
Wire Wire Line
	10450 4900 10550 4900
Wire Wire Line
	10450 4800 10550 4800
Wire Wire Line
	10450 4700 10550 4700
Wire Wire Line
	10450 4600 10550 4600
Wire Wire Line
	10450 4500 10550 4500
Wire Wire Line
	10450 4400 10550 4400
Wire Wire Line
	10450 4300 10550 4300
Wire Wire Line
	10450 3800 10550 3800
Text GLabel 10550 5100 2    50   Input ~ 0
A0
Text GLabel 10550 5000 2    50   Input ~ 0
A1
Text GLabel 10550 4900 2    50   Input ~ 0
A2
Text GLabel 10550 4800 2    50   Input ~ 0
A3
Text GLabel 10550 4600 2    50   Input ~ 0
A5
Text GLabel 10550 4500 2    50   Input ~ 0
A6
Text GLabel 10550 4400 2    50   Input ~ 0
A7
Text GLabel 10550 4300 2    50   Input ~ 0
A8
Text GLabel 10550 4200 2    50   Input ~ 0
A9
Text GLabel 10550 3800 2    50   Input ~ 0
VIN
$Comp
L power:+3.3V #PWR028
U 1 1 5CF45684
P 11100 3950
F 0 "#PWR028" H 11100 3800 50  0001 C CNN
F 1 "+3.3V" H 11115 4123 50  0000 C CNN
F 2 "" H 11100 3950 50  0001 C CNN
F 3 "" H 11100 3950 50  0001 C CNN
	1    11100 3950
	1    0    0    -1  
$EndComp
Text Notes 9050 2850 0    50   ~ 0
Headers
$Comp
L Switch:SW_Push SW1
U 1 1 5CF4F6EA
P 4650 1250
F 0 "SW1" H 4650 1535 50  0000 C CNN
F 1 "SW_Push" H 4650 1444 50  0000 C CNN
F 2 "Button_Switch_SMD:SW_SPST_PTS810" H 4650 1450 50  0001 C CNN
F 3 "~" H 4650 1450 50  0001 C CNN
	1    4650 1250
	1    0    0    -1  
$EndComp
Wire Wire Line
	2000 3500 1900 3500
Wire Wire Line
	2000 3600 1900 3600
Text GLabel 1900 3500 0    50   Input ~ 0
BOOT0
Text GLabel 1900 3600 0    50   Input ~ 0
RESET
$Comp
L power:GND #PWR015
U 1 1 5CF54315
P 4950 1750
F 0 "#PWR015" H 4950 1500 50  0001 C CNN
F 1 "GND" H 4955 1577 50  0000 C CNN
F 2 "" H 4950 1750 50  0001 C CNN
F 3 "" H 4950 1750 50  0001 C CNN
	1    4950 1750
	1    0    0    -1  
$EndComp
Wire Wire Line
	4850 1250 4950 1250
Text GLabel 4250 1250 0    50   Input ~ 0
RESET
Wire Wire Line
	4250 1250 4350 1250
$Comp
L Device:C C6
U 1 1 5CF6DF1F
P 4350 1500
F 0 "C6" H 4465 1546 50  0000 L CNN
F 1 "100nF" H 4465 1455 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 4388 1350 50  0001 C CNN
F 3 "~" H 4350 1500 50  0001 C CNN
	1    4350 1500
	1    0    0    -1  
$EndComp
Wire Wire Line
	4950 1250 4950 1750
Wire Wire Line
	4350 1250 4350 1350
Connection ~ 4350 1250
Wire Wire Line
	4350 1250 4450 1250
$Comp
L power:GND #PWR012
U 1 1 5CF721CF
P 4350 1750
F 0 "#PWR012" H 4350 1500 50  0001 C CNN
F 1 "GND" H 4355 1577 50  0000 C CNN
F 2 "" H 4350 1750 50  0001 C CNN
F 3 "" H 4350 1750 50  0001 C CNN
	1    4350 1750
	1    0    0    -1  
$EndComp
Wire Wire Line
	4350 1650 4350 1750
$Comp
L Switch:SW_Push SW2
U 1 1 5CF765E0
P 5950 1250
F 0 "SW2" H 5950 1535 50  0000 C CNN
F 1 "SW_Push" H 5950 1444 50  0000 C CNN
F 2 "Button_Switch_SMD:SW_SPST_PTS810" H 5950 1450 50  0001 C CNN
F 3 "~" H 5950 1450 50  0001 C CNN
	1    5950 1250
	1    0    0    -1  
$EndComp
Wire Wire Line
	5750 1250 5650 1250
Wire Wire Line
	5650 1250 5650 1350
Connection ~ 5650 1250
Wire Wire Line
	5550 1250 5650 1250
Text GLabel 5550 1250 0    50   Input ~ 0
BOOT0
$Comp
L Device:R R1
U 1 1 5CF7C8B8
P 5650 1500
F 0 "R1" H 5720 1546 50  0000 L CNN
F 1 "10k" H 5720 1455 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 5580 1500 50  0001 C CNN
F 3 "~" H 5650 1500 50  0001 C CNN
	1    5650 1500
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR016
U 1 1 5CF7CFE3
P 5650 1750
F 0 "#PWR016" H 5650 1500 50  0001 C CNN
F 1 "GND" H 5655 1577 50  0000 C CNN
F 2 "" H 5650 1750 50  0001 C CNN
F 3 "" H 5650 1750 50  0001 C CNN
	1    5650 1750
	1    0    0    -1  
$EndComp
Wire Wire Line
	5650 1650 5650 1750
$Comp
L power:+3.3V #PWR017
U 1 1 5CF7F5B9
P 6250 1150
F 0 "#PWR017" H 6250 1000 50  0001 C CNN
F 1 "+3.3V" H 6265 1323 50  0000 C CNN
F 2 "" H 6250 1150 50  0001 C CNN
F 3 "" H 6250 1150 50  0001 C CNN
	1    6250 1150
	1    0    0    -1  
$EndComp
Wire Wire Line
	6150 1250 6250 1250
Wire Wire Line
	6250 1250 6250 1150
Text Notes 3800 600  0    50   ~ 0
Buttons
Text GLabel 1900 3800 0    50   Input ~ 0
A0
Wire Wire Line
	2000 3800 1900 3800
Wire Wire Line
	1900 3900 2000 3900
Wire Wire Line
	2000 4000 1900 4000
Wire Wire Line
	1900 4100 2000 4100
Wire Wire Line
	2000 4200 1900 4200
Wire Wire Line
	1900 4300 2000 4300
Wire Wire Line
	2000 4400 1900 4400
Wire Wire Line
	2000 4500 1900 4500
Wire Wire Line
	2000 4700 1900 4700
Wire Wire Line
	1900 4800 2000 4800
Wire Wire Line
	2000 4900 1900 4900
Wire Wire Line
	1900 5000 2000 5000
Wire Wire Line
	2000 5100 1900 5100
Wire Wire Line
	1900 5200 2000 5200
Wire Wire Line
	2000 5300 1900 5300
Wire Wire Line
	2000 5500 1900 5500
Wire Wire Line
	2000 5700 1900 5700
Wire Wire Line
	7600 3200 7700 3200
Wire Wire Line
	7600 3800 7700 3800
Wire Wire Line
	7700 3900 7600 3900
Wire Wire Line
	7700 4100 7600 4100
Wire Wire Line
	7600 4200 7700 4200
Wire Wire Line
	7700 4300 7600 4300
Wire Wire Line
	7600 4400 7700 4400
Wire Wire Line
	7700 4500 7600 4500
Wire Wire Line
	7600 4600 7700 4600
Wire Wire Line
	7600 4700 7700 4700
Wire Wire Line
	7700 5000 7600 5000
Wire Wire Line
	7600 5100 7700 5100
Wire Wire Line
	7700 5200 7600 5200
Wire Wire Line
	7600 5300 7700 5300
Text GLabel 1900 3900 0    50   Input ~ 0
A1
Text GLabel 1900 4000 0    50   Input ~ 0
A2
Text GLabel 1900 4100 0    50   Input ~ 0
A3
Text GLabel 1900 4200 0    50   Input ~ 0
A4
Text GLabel 1900 4300 0    50   Input ~ 0
A5
Text GLabel 1900 4400 0    50   Input ~ 0
A6
Text GLabel 1900 4500 0    50   Input ~ 0
A7
Text GLabel 1900 4700 0    50   Input ~ 0
D11
Text GLabel 1900 4800 0    50   Input ~ 0
D10
Text GLabel 1900 4900 0    50   Input ~ 0
USB_D-
Text GLabel 1900 5000 0    50   Input ~ 0
USB_D+
Text GLabel 1900 5100 0    50   Input ~ 0
SWDIO
Text GLabel 1900 5200 0    50   Input ~ 0
SWCLK
Text GLabel 1900 5300 0    50   Input ~ 0
D0
Text GLabel 1900 5500 0    50   Input ~ 0
D20
$Comp
L power:+3.3V #PWR021
U 1 1 5D039F85
P 7700 3100
F 0 "#PWR021" H 7700 2950 50  0001 C CNN
F 1 "+3.3V" H 7715 3273 50  0000 C CNN
F 2 "" H 7700 3100 50  0001 C CNN
F 3 "" H 7700 3100 50  0001 C CNN
	1    7700 3100
	1    0    0    -1  
$EndComp
Wire Wire Line
	7700 3200 7700 3100
$Comp
L dk_Ferrite-Beads-and-Chips:BLM18AG601SN1D FB1
U 1 1 5D041486
P 1500 7000
F 0 "FB1" H 1500 7287 60  0000 C CNN
F 1 "BLM18AG601SN1D" H 1500 7181 60  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" H 1700 7200 60  0001 L CNN
F 3 "https://www.murata.com/en-us/products/productdata/8796738650142/ENFA0003.pdf" H 1700 7300 60  0001 L CNN
F 4 "490-1014-1-ND" H 1700 7400 60  0001 L CNN "Digi-Key_PN"
F 5 "BLM18AG601SN1D" H 1700 7500 60  0001 L CNN "MPN"
F 6 "Filters" H 1700 7600 60  0001 L CNN "Category"
F 7 "Ferrite Beads and Chips" H 1700 7700 60  0001 L CNN "Family"
F 8 "https://www.murata.com/en-us/products/productdata/8796738650142/ENFA0003.pdf" H 1700 7800 60  0001 L CNN "DK_Datasheet_Link"
F 9 "/product-detail/en/murata-electronics-north-america/BLM18AG601SN1D/490-1014-1-ND/584462" H 1700 7900 60  0001 L CNN "DK_Detail_Page"
F 10 "FERRITE BEAD 600 OHM 0603 1LN" H 1700 8000 60  0001 L CNN "Description"
F 11 "Murata Electronics North America" H 1700 8100 60  0001 L CNN "Manufacturer"
F 12 "Active" H 1700 8200 60  0001 L CNN "Status"
	1    1500 7000
	1    0    0    -1  
$EndComp
Wire Wire Line
	7600 3300 7700 3300
Text GLabel 7700 3300 2    50   Input ~ 0
AREF
Text GLabel 950  7000 0    50   Input ~ 0
AREF
$Comp
L Device:C C3
U 1 1 5D063956
P 1050 7250
F 0 "C3" H 1165 7296 50  0000 L CNN
F 1 "100nF" H 1165 7205 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 1088 7100 50  0001 C CNN
F 3 "~" H 1050 7250 50  0001 C CNN
	1    1050 7250
	1    0    0    -1  
$EndComp
Wire Wire Line
	950  7000 1050 7000
Wire Wire Line
	1050 7000 1050 7100
Connection ~ 1050 7000
Wire Wire Line
	1050 7000 1300 7000
$Comp
L Jumper:SolderJumper_2_Bridged JP1
U 1 1 5D07020A
P 1950 7000
F 0 "JP1" H 1900 6850 50  0000 L CNN
F 1 "SolderJumper_2_Bridged" H 1500 6800 50  0000 L CNN
F 2 "Jumper:SolderJumper-2_P1.3mm_Bridged_RoundedPad1.0x1.5mm" H 1950 7000 50  0001 C CNN
F 3 "~" H 1950 7000 50  0001 C CNN
	1    1950 7000
	1    0    0    -1  
$EndComp
Wire Wire Line
	1700 7000 1800 7000
$Comp
L power:+3.3V #PWR05
U 1 1 5D0779DC
P 2200 6900
F 0 "#PWR05" H 2200 6750 50  0001 C CNN
F 1 "+3.3V" H 2215 7073 50  0000 C CNN
F 2 "" H 2200 6900 50  0001 C CNN
F 3 "" H 2200 6900 50  0001 C CNN
	1    2200 6900
	1    0    0    -1  
$EndComp
Wire Wire Line
	2100 7000 2200 7000
Wire Wire Line
	2200 7000 2200 6900
Text Notes 900  6600 0    50   ~ 0
Connect VDDA to VDD\nCut jumper to use separate VDDA
$Comp
L power:GNDA #PWR04
U 1 1 5D07F633
P 1050 7500
F 0 "#PWR04" H 1050 7250 50  0001 C CNN
F 1 "GNDA" H 1055 7327 50  0000 C CNN
F 2 "" H 1050 7500 50  0001 C CNN
F 3 "" H 1050 7500 50  0001 C CNN
	1    1050 7500
	1    0    0    -1  
$EndComp
Wire Wire Line
	1050 7400 1050 7500
Text Notes 2600 6600 0    50   ~ 0
Connect VSSA to VSS\nCut jumper to use separate VSSA
Wire Wire Line
	3350 7000 3350 7100
Wire Wire Line
	3250 7000 3350 7000
Wire Wire Line
	2850 7000 2850 7100
Wire Wire Line
	2950 7000 2850 7000
$Comp
L power:GND #PWR011
U 1 1 5D08DFE9
P 3350 7100
F 0 "#PWR011" H 3350 6850 50  0001 C CNN
F 1 "GND" H 3355 6927 50  0000 C CNN
F 2 "" H 3350 7100 50  0001 C CNN
F 3 "" H 3350 7100 50  0001 C CNN
	1    3350 7100
	1    0    0    -1  
$EndComp
$Comp
L Jumper:SolderJumper_2_Bridged JP2
U 1 1 5D08DA23
P 3100 7000
F 0 "JP2" H 3000 7150 50  0000 L CNN
F 1 "SolderJumper_2_Bridged" H 2650 7100 50  0000 L CNN
F 2 "Jumper:SolderJumper-2_P1.3mm_Bridged_RoundedPad1.0x1.5mm" H 3100 7000 50  0001 C CNN
F 3 "~" H 3100 7000 50  0001 C CNN
	1    3100 7000
	1    0    0    -1  
$EndComp
$Comp
L power:GNDA #PWR07
U 1 1 5D08D64E
P 2850 7100
F 0 "#PWR07" H 2850 6850 50  0001 C CNN
F 1 "GNDA" H 2855 6927 50  0000 C CNN
F 2 "" H 2850 7100 50  0001 C CNN
F 3 "" H 2850 7100 50  0001 C CNN
	1    2850 7100
	1    0    0    -1  
$EndComp
$Comp
L power:GNDA #PWR022
U 1 1 5D0AAD3A
P 7650 6100
F 0 "#PWR022" H 7650 5850 50  0001 C CNN
F 1 "GNDA" H 7655 5927 50  0000 C CNN
F 2 "" H 7650 6100 50  0001 C CNN
F 3 "" H 7650 6100 50  0001 C CNN
	1    7650 6100
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR023
U 1 1 5D0AB1A1
P 7850 6100
F 0 "#PWR023" H 7850 5850 50  0001 C CNN
F 1 "GND" H 7855 5927 50  0000 C CNN
F 2 "" H 7850 6100 50  0001 C CNN
F 3 "" H 7850 6100 50  0001 C CNN
	1    7850 6100
	1    0    0    -1  
$EndComp
$Comp
L Device:C C7
U 1 1 5D0B93B9
P 4350 7150
F 0 "C7" H 4465 7196 50  0000 L CNN
F 1 "100nF" H 4465 7105 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 4388 7000 50  0001 C CNN
F 3 "~" H 4350 7150 50  0001 C CNN
	1    4350 7150
	1    0    0    -1  
$EndComp
$Comp
L Device:C C9
U 1 1 5D0B9E44
P 5350 7150
F 0 "C9" H 5465 7196 50  0000 L CNN
F 1 "100nF" H 5465 7105 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 5388 7000 50  0001 C CNN
F 3 "~" H 5350 7150 50  0001 C CNN
	1    5350 7150
	1    0    0    -1  
$EndComp
$Comp
L Device:C C10
U 1 1 5D0BA150
P 5850 7150
F 0 "C10" H 5965 7196 50  0000 L CNN
F 1 "100nF" H 5965 7105 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 5888 7000 50  0001 C CNN
F 3 "~" H 5850 7150 50  0001 C CNN
	1    5850 7150
	1    0    0    -1  
$EndComp
$Comp
L Device:C C8
U 1 1 5D0B9B61
P 4850 7150
F 0 "C8" H 4965 7196 50  0000 L CNN
F 1 "100nF" H 4965 7105 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 4888 7000 50  0001 C CNN
F 3 "~" H 4850 7150 50  0001 C CNN
	1    4850 7150
	1    0    0    -1  
$EndComp
$Comp
L power:+3.3V #PWR013
U 1 1 5D0BB3CD
P 4350 6800
F 0 "#PWR013" H 4350 6650 50  0001 C CNN
F 1 "+3.3V" H 4365 6973 50  0000 C CNN
F 2 "" H 4350 6800 50  0001 C CNN
F 3 "" H 4350 6800 50  0001 C CNN
	1    4350 6800
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR014
U 1 1 5D0BBCDF
P 4350 7500
F 0 "#PWR014" H 4350 7250 50  0001 C CNN
F 1 "GND" H 4355 7327 50  0000 C CNN
F 2 "" H 4350 7500 50  0001 C CNN
F 3 "" H 4350 7500 50  0001 C CNN
	1    4350 7500
	1    0    0    -1  
$EndComp
Wire Wire Line
	4350 6800 4350 6900
Wire Wire Line
	4350 7300 4350 7400
Wire Wire Line
	4350 6900 4850 6900
Wire Wire Line
	4850 6900 4850 7000
Connection ~ 4350 6900
Wire Wire Line
	4350 6900 4350 7000
Wire Wire Line
	4850 6900 5350 6900
Wire Wire Line
	5350 6900 5350 7000
Connection ~ 4850 6900
Wire Wire Line
	5350 6900 5850 6900
Wire Wire Line
	5850 6900 5850 7000
Connection ~ 5350 6900
Wire Wire Line
	4850 7300 4850 7400
Connection ~ 4350 7400
Wire Wire Line
	4350 7400 4350 7500
Wire Wire Line
	5350 7300 5350 7400
Connection ~ 4850 7400
Wire Wire Line
	5850 7300 5850 7400
Connection ~ 5350 7400
Text GLabel 7700 3800 2    50   Input ~ 0
A8
Text GLabel 7700 3900 2    50   Input ~ 0
A9
NoConn ~ 7600 4000
Text GLabel 7700 4100 2    50   Input ~ 0
D1
Text GLabel 7700 4200 2    50   Input ~ 0
D2
Text GLabel 7700 4300 2    50   Input ~ 0
D3
Text GLabel 7700 4400 2    50   Input ~ 0
D4
Text GLabel 7700 4500 2    50   Input ~ 0
D5
Text GLabel 7700 4600 2    50   Input ~ 0
D6
Text GLabel 7700 4700 2    50   Input ~ 0
D7
Text GLabel 7700 5000 2    50   Input ~ 0
D16
Text GLabel 7700 5100 2    50   Input ~ 0
D15
Text GLabel 7700 5200 2    50   Input ~ 0
D14
Text GLabel 7700 5300 2    50   Input ~ 0
D13
$Comp
L Device:C C11
U 1 1 5D12187B
P 8650 5450
F 0 "C11" V 8398 5450 50  0000 C CNN
F 1 "10pF" V 8489 5450 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 8688 5300 50  0001 C CNN
F 3 "~" H 8650 5450 50  0001 C CNN
	1    8650 5450
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR025
U 1 1 5D159F33
P 8850 6100
F 0 "#PWR025" H 8850 5850 50  0001 C CNN
F 1 "GND" H 8855 5927 50  0000 C CNN
F 2 "" H 8850 6100 50  0001 C CNN
F 3 "" H 8850 6100 50  0001 C CNN
	1    8850 6100
	1    0    0    -1  
$EndComp
$Comp
L Device:Crystal Y1
U 1 1 5D164E61
P 1300 5800
F 0 "Y1" V 1254 5931 50  0000 L CNN
F 1 "32.768kHZ" V 1345 5931 50  0000 L CNN
F 2 "Crystal:Crystal_SMD_G8-2Pin_3.2x1.5mm" H 1300 5800 50  0001 C CNN
F 3 "~" H 1300 5800 50  0001 C CNN
F 4 "DNP" V 1150 6000 50  0000 C CNN "Config"
	1    1300 5800
	0    1    1    0   
$EndComp
$Comp
L Device:C C1
U 1 1 5D189937
P 1050 5600
F 0 "C1" V 798 5600 50  0000 C CNN
F 1 "15pF" V 889 5600 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 1088 5450 50  0001 C CNN
F 3 "~" H 1050 5600 50  0001 C CNN
F 4 "DNP" H 950 5450 50  0000 C CNN "Config"
	1    1050 5600
	0    1    1    0   
$EndComp
Wire Wire Line
	1200 5600 1300 5600
Wire Wire Line
	1300 5650 1300 5600
Connection ~ 1300 5600
Wire Wire Line
	1300 5600 2000 5600
Wire Wire Line
	1200 6000 1300 6000
Wire Wire Line
	1900 6000 1900 5700
Wire Wire Line
	1300 5950 1300 6000
Connection ~ 1300 6000
Wire Wire Line
	1300 6000 1900 6000
$Comp
L power:GND #PWR02
U 1 1 5D1AD292
P 850 6100
F 0 "#PWR02" H 850 5850 50  0001 C CNN
F 1 "GND" H 855 5927 50  0000 C CNN
F 2 "" H 850 6100 50  0001 C CNN
F 3 "" H 850 6100 50  0001 C CNN
	1    850  6100
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR01
U 1 1 5D1ADA16
P 650 6100
F 0 "#PWR01" H 650 5850 50  0001 C CNN
F 1 "GND" H 655 5927 50  0000 C CNN
F 2 "" H 650 6100 50  0001 C CNN
F 3 "" H 650 6100 50  0001 C CNN
	1    650  6100
	1    0    0    -1  
$EndComp
Wire Wire Line
	900  6000 850  6000
Wire Wire Line
	850  6000 850  6100
Wire Wire Line
	900  5600 650  5600
Wire Wire Line
	650  5600 650  6100
Wire Wire Line
	7600 5600 7650 5600
Wire Wire Line
	7650 5600 7650 5450
Wire Wire Line
	7600 5700 7650 5700
Wire Wire Line
	7650 5700 7650 5850
Wire Notes Line
	11200 2700 500  2700
Wire Notes Line
	3750 500  3750 2700
Wire Notes Line
	6600 500  6600 2700
Wire Notes Line
	9350 500  9350 2700
Wire Notes Line
	9000 2700 9000 6500
Text Notes 550  2850 0    50   ~ 0
Microcontroller
$Comp
L Mechanical:Fiducial FID1
U 1 1 5CF5F029
P 10750 6700
F 0 "FID1" H 10835 6746 50  0000 L CNN
F 1 "Fiducial" H 10835 6655 50  0000 L CNN
F 2 "Fiducial:Fiducial_1mm_Mask2mm" H 10750 6700 50  0001 C CNN
F 3 "~" H 10750 6700 50  0001 C CNN
	1    10750 6700
	1    0    0    -1  
$EndComp
$Comp
L Mechanical:Fiducial FID2
U 1 1 5CF5FC1A
P 10750 6950
F 0 "FID2" H 10835 6996 50  0000 L CNN
F 1 "Fiducial" H 10835 6905 50  0000 L CNN
F 2 "Fiducial:Fiducial_1mm_Mask2mm" H 10750 6950 50  0001 C CNN
F 3 "~" H 10750 6950 50  0001 C CNN
	1    10750 6950
	1    0    0    -1  
$EndComp
$Comp
L Device:C C12
U 1 1 5D12089B
P 8650 5850
F 0 "C12" V 8398 5850 50  0000 C CNN
F 1 "10pF" V 8489 5850 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 8688 5700 50  0001 C CNN
F 3 "~" H 8650 5850 50  0001 C CNN
	1    8650 5850
	0    1    1    0   
$EndComp
Wire Wire Line
	7600 6000 7650 6000
Wire Wire Line
	7650 6000 7650 6100
Wire Wire Line
	7600 5900 7850 5900
Wire Wire Line
	7850 5900 7850 6100
$Comp
L Device:Crystal_GND24 Y2
U 1 1 5CFCA91C
P 8050 5650
F 0 "Y2" V 7900 5450 50  0000 L CNN
F 1 "16MHz" V 7900 5700 50  0000 L CNN
F 2 "Crystal:Crystal_SMD_SeikoEpson_TSX3225-4Pin_3.2x2.5mm" H 8050 5650 50  0001 C CNN
F 3 "~" H 8050 5650 50  0001 C CNN
	1    8050 5650
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0101
U 1 1 5D01138E
P 7800 5650
F 0 "#PWR0101" H 7800 5400 50  0001 C CNN
F 1 "GND" V 7900 5600 50  0000 C CNN
F 2 "" H 7800 5650 50  0001 C CNN
F 3 "" H 7800 5650 50  0001 C CNN
	1    7800 5650
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0102
U 1 1 5D011927
P 8300 5650
F 0 "#PWR0102" H 8300 5400 50  0001 C CNN
F 1 "GND" V 8200 5600 50  0000 C CNN
F 2 "" H 8300 5650 50  0001 C CNN
F 3 "" H 8300 5650 50  0001 C CNN
	1    8300 5650
	0    -1   -1   0   
$EndComp
Wire Wire Line
	7650 5450 8050 5450
Wire Wire Line
	7650 5850 8050 5850
Wire Wire Line
	7850 5650 7800 5650
Wire Wire Line
	8250 5650 8300 5650
Wire Wire Line
	8050 5500 8050 5450
Connection ~ 8050 5450
Wire Wire Line
	8050 5800 8050 5850
Connection ~ 8050 5850
Wire Wire Line
	8050 5450 8500 5450
Wire Wire Line
	8050 5850 8500 5850
Wire Wire Line
	8800 5450 8850 5450
Wire Wire Line
	8850 5450 8850 5850
Wire Wire Line
	8800 5850 8850 5850
Connection ~ 8850 5850
Wire Wire Line
	8850 5850 8850 6100
Text GLabel 9600 3900 0    50   Input ~ 0
D0
Text GLabel 9600 4000 0    50   Input ~ 0
D1
Text GLabel 9600 4100 0    50   Input ~ 0
D2
Text GLabel 9600 4200 0    50   Input ~ 0
D3
Text GLabel 9600 4300 0    50   Input ~ 0
D4
Text GLabel 9600 4400 0    50   Input ~ 0
D5
Text GLabel 9600 4500 0    50   Input ~ 0
D6
Text GLabel 9600 4600 0    50   Input ~ 0
D7
Text GLabel 9600 4700 0    50   Input ~ 0
D10
Text GLabel 9600 4800 0    50   Input ~ 0
D11
Text GLabel 9600 4900 0    50   Input ~ 0
D13
Text GLabel 9600 5000 0    50   Input ~ 0
D14
Text GLabel 9600 5100 0    50   Input ~ 0
D15
Text GLabel 9600 5200 0    50   Input ~ 0
D16
$Comp
L power:GNDA #PWR0103
U 1 1 5D02AF3F
P 10900 3750
F 0 "#PWR0103" H 10900 3500 50  0001 C CNN
F 1 "GNDA" H 10905 3577 50  0000 C CNN
F 2 "" H 10900 3750 50  0001 C CNN
F 3 "" H 10900 3750 50  0001 C CNN
	1    10900 3750
	1    0    0    -1  
$EndComp
Wire Wire Line
	10450 4100 10550 4100
Text GLabel 10550 4100 2    50   Input ~ 0
AREF
Text GLabel 10550 4700 2    50   Input ~ 0
A4
$Comp
L power:GND #PWR0104
U 1 1 5D05FB59
P 9300 3900
F 0 "#PWR0104" H 9300 3650 50  0001 C CNN
F 1 "GND" H 9305 3727 50  0000 C CNN
F 2 "" H 9300 3900 50  0001 C CNN
F 3 "" H 9300 3900 50  0001 C CNN
	1    9300 3900
	1    0    0    -1  
$EndComp
Wire Wire Line
	9300 3800 9300 3900
Wire Wire Line
	9300 3800 9700 3800
$Comp
L Connector_Generic:Conn_01x16 J2
U 1 1 5D06D550
P 9900 4500
F 0 "J2" H 9850 5400 50  0000 L CNN
F 1 "Conn_01x16" H 9567 5315 50  0000 L CNN
F 2 "STM32F070_Breakout:PinHeader_1x16_P2.54mm_Vertical_NoSilk" H 9900 4500 50  0001 C CNN
F 3 "~" H 9900 4500 50  0001 C CNN
F 4 "DNP" H 9900 5500 50  0000 C CNN "Config"
	1    9900 4500
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x16 J3
U 1 1 5D06E7A9
P 10250 4500
F 0 "J3" H 10157 5394 50  0000 C CNN
F 1 "Conn_01x16" H 10157 5315 50  0000 C CNN
F 2 "STM32F070_Breakout:PinHeader_1x16_P2.54mm_Vertical_NoSilk" H 10250 4500 50  0001 C CNN
F 3 "~" H 10250 4500 50  0001 C CNN
F 4 "DNP" H 10150 5500 50  0000 C CNN "Config"
	1    10250 4500
	-1   0    0    -1  
$EndComp
Wire Wire Line
	10450 3900 10800 3900
Wire Wire Line
	10800 3900 10800 3700
Wire Wire Line
	10800 3700 10900 3700
Wire Wire Line
	10900 3700 10900 3750
Wire Wire Line
	11100 4000 11100 3950
Wire Wire Line
	10450 4000 11100 4000
Wire Wire Line
	10450 5300 10550 5300
Wire Wire Line
	9700 5300 9600 5300
Text GLabel 10550 5200 2    50   Input ~ 0
D20
Text GLabel 10550 5300 2    50   Input ~ 0
RESET
Text GLabel 9600 5300 0    50   Input ~ 0
BOOT0
Wire Wire Line
	10450 4200 10550 4200
$Comp
L Graphic:Logo_Open_Hardware_Small #LOGO1
U 1 1 5CFB14F5
P 10300 6850
F 0 "#LOGO1" H 10300 7125 50  0001 C CNN
F 1 "Logo_Open_Hardware_Small" H 10300 6625 50  0001 C CNN
F 2 "" H 10300 6850 50  0001 C CNN
F 3 "~" H 10300 6850 50  0001 C CNN
	1    10300 6850
	1    0    0    -1  
$EndComp
$Comp
L dk_USB-DVI-HDMI-Connectors:10118194-0001LF J1
U 1 1 5CFB603F
P 950 1450
F 0 "J1" H 1014 2195 60  0000 C CNN
F 1 "10118194-0001LF" H 1014 2089 60  0000 C CNN
F 2 "STM32F070_Breakout:USB_Micro_B_Female_10118194-0001LF_oversized_pads" H 1150 1650 60  0001 L CNN
F 3 "http://www.amphenol-icc.com/media/wysiwyg/files/drawing/10118194.pdf" H 1150 1750 60  0001 L CNN
F 4 "609-4618-1-ND" H 1150 1850 60  0001 L CNN "Digi-Key_PN"
F 5 "10118194-0001LF" H 1150 1950 60  0001 L CNN "MPN"
F 6 "Connectors, Interconnects" H 1150 2050 60  0001 L CNN "Category"
F 7 "USB, DVI, HDMI Connectors" H 1150 2150 60  0001 L CNN "Family"
F 8 "http://www.amphenol-icc.com/media/wysiwyg/files/drawing/10118194.pdf" H 1150 2250 60  0001 L CNN "DK_Datasheet_Link"
F 9 "/product-detail/en/amphenol-icc-fci/10118194-0001LF/609-4618-1-ND/2785382" H 1150 2350 60  0001 L CNN "DK_Detail_Page"
F 10 "CONN RCPT USB2.0 MICRO B SMD R/A" H 1150 2450 60  0001 L CNN "Description"
F 11 "Amphenol ICC (FCI)" H 1150 2550 60  0001 L CNN "Manufacturer"
F 12 "Active" H 1150 2650 60  0001 L CNN "Status"
	1    950  1450
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR03
U 1 1 5CF140F9
P 850 2150
F 0 "#PWR03" H 850 1900 50  0001 C CNN
F 1 "GND" H 855 1977 50  0000 C CNN
F 2 "" H 850 2150 50  0001 C CNN
F 3 "" H 850 2150 50  0001 C CNN
	1    850  2150
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0105
U 1 1 5CFB92F0
P 1350 1750
F 0 "#PWR0105" H 1350 1500 50  0001 C CNN
F 1 "GND" H 1355 1577 50  0000 C CNN
F 2 "" H 1350 1750 50  0001 C CNN
F 3 "" H 1350 1750 50  0001 C CNN
	1    1350 1750
	1    0    0    -1  
$EndComp
Wire Wire Line
	1250 1350 1350 1350
Wire Wire Line
	1250 1450 1350 1450
Wire Wire Line
	1250 1650 1350 1650
Wire Wire Line
	1350 1650 1350 1750
Wire Wire Line
	850  2050 850  2150
NoConn ~ 1250 1550
Text Notes 2650 900  0    50   ~ 0
Vin: 3.5 - 6.0 V\nImax: 600 mA
Wire Wire Line
	2000 4600 1900 4600
Text GLabel 1900 4600 0    50   Input ~ 0
D12
$Comp
L Connector_Generic:Conn_01x01 J5
U 1 1 5D492255
P 10400 5750
F 0 "J5" H 10480 5792 50  0000 L CNN
F 1 "Conn_01x01" H 10480 5701 50  0000 L CNN
F 2 "STM32F070_Breakout:TestPoint_Pad_1.0x2.5mm" H 10400 5750 50  0001 C CNN
F 3 "~" H 10400 5750 50  0001 C CNN
	1    10400 5750
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x01 J6
U 1 1 5D492A9F
P 10400 5950
F 0 "J6" H 10480 5992 50  0000 L CNN
F 1 "Conn_01x01" H 10480 5901 50  0000 L CNN
F 2 "STM32F070_Breakout:TestPoint_Pad_1.0x2.5mm" H 10400 5950 50  0001 C CNN
F 3 "~" H 10400 5950 50  0001 C CNN
	1    10400 5950
	1    0    0    -1  
$EndComp
Wire Wire Line
	10200 5750 10100 5750
Wire Wire Line
	10200 5950 10100 5950
Text GLabel 10100 5750 0    50   Input ~ 0
D12
Wire Wire Line
	7600 4900 7700 4900
Text GLabel 7700 4900 2    50   Input ~ 0
D31
Text GLabel 10100 5950 0    50   Input ~ 0
D30
$Comp
L Connector_Generic:Conn_01x01 J7
U 1 1 5D519CA9
P 10400 6150
F 0 "J7" H 10480 6192 50  0000 L CNN
F 1 "Conn_01x01" H 10480 6101 50  0000 L CNN
F 2 "STM32F070_Breakout:TestPoint_Pad_1.0x2.5mm" H 10400 6150 50  0001 C CNN
F 3 "~" H 10400 6150 50  0001 C CNN
	1    10400 6150
	1    0    0    -1  
$EndComp
Wire Wire Line
	10200 6150 10100 6150
Text GLabel 10100 6150 0    50   Input ~ 0
D31
Wire Wire Line
	7600 4800 7700 4800
Text GLabel 7700 4800 2    50   Input ~ 0
D30
Wire Wire Line
	5350 7400 4850 7400
Wire Wire Line
	4850 7400 4350 7400
Wire Wire Line
	5850 7400 5350 7400
$Comp
L STM32F070CBT6:STM32F070CBT6 U2
U 1 1 5CF0F9EF
P 4800 4600
F 0 "U2" H 4800 6270 50  0000 C CNN
F 1 "STM32F070CBT6" H 4800 6179 50  0000 C CNN
F 2 "STM32F070CBT6:QFP50P900X900X160-48N" H 4800 4600 50  0001 L BNN
F 3 "" H 4800 4600 50  0001 L BNN
F 4 "Entry-level ARM Cortex-M0 Value line MCU with up to 128 Kbytes Flash, 48 MHz CPU" H 4800 4600 50  0001 L BNN "Field4"
F 5 "STMicroelectronics" H 4800 4600 50  0001 L BNN "Field5"
F 6 "None" H 4800 4600 50  0001 L BNN "Field6"
F 7 "STM32F070CBT6" H 4800 4600 50  0001 L BNN "Field7"
F 8 "LQFP-48 STMicroelectronics" H 4800 4600 50  0001 L BNN "Field8"
	1    4800 4600
	1    0    0    -1  
$EndComp
$Comp
L Device:C C2
U 1 1 5D187B26
P 1050 6000
F 0 "C2" V 798 6000 50  0000 C CNN
F 1 "15pF" V 889 6000 50  0000 C CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 1088 5850 50  0001 C CNN
F 3 "~" H 1050 6000 50  0001 C CNN
F 4 "DNP" H 950 5850 50  0000 C CNN "Config"
	1    1050 6000
	0    1    1    0   
$EndComp
$EndSCHEMATC
